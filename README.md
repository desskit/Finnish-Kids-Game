...
- **`animals.sourced.json`**, **`numbers.sourced.json`**, **`adjectives.sourced.json`**
  (under `src/content/data/`) — each word carries its full inflection table
  (`nominative_singular`, `genitive_singular`, `elative_singular`, …), plus
  `kotusType`, `group`, `frequencyRank`, and example sentences. Adjectives come
  from the same source (they decline like nouns). **CC BY-SA 4.0** — see `NOTICE.md`.
...
Edit the curation lists (`ANIMALS`, `NUMBERS`, `ADJECTIVES`) in `scripts/build-kids-data.mjs`
